  ...
  forward(req: Request, res: Response, id: string) {
    ...
    if (!route) {
      throw new NotFoundException(
        'No route config found in gateway config files, please check it.',
      );
    }
    if (route.uri.indexOf('lb://') === 0) {
      ...
      if (!server) {
        throw new InternalServerErrorException(
          `No available server can handle this request`,
        );
      }
      const target = `${this.proxyOptions.secure ? 'https' : 'http'}://${
        server.address
      }:${server.port}${req.url}`;
      ...
      route.uri.indexOf('http://') === 0 ||
      route.uri.indexOf('https://') === 0
    ) {
