...
import { ConsoleTransport, DailyRotateFile, FileTransport, LoggerOptions } from "./LoggerOptions";
        ...
        const transports = this.options.transports
            ...
            .map(item => {
                if (item.transport === 'console') {
                    const config = item as ConsoleTransport;
                    return new winston.transports.Console({
                        colorize: config.colorize,
                        label: config.label,
                        timestamp: () => moment(new Date().getTime()).format(config.datePattern || 'YYYY-MM-DD h:mm:ss'),
                    })
                } else if (item.transport === 'file') {
                    const config = item as FileTransport;
                    ...
                    const config = item as DailyRotateFile;
